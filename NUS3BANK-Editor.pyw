...
appVer = u'2.20-pre'
appName = u"NUS3BANK Editor (Sony AT9 fork) " + appVer    
...
at9   = u'"' + os.path.join(os.path.join(n3beDir,u'exeLib'), u'at9tool.exe') + u'"'
...
def convert2idsp(replacementSound):
    ...
    if subprocess.call(xis.encode(cmd)):
        if os.path.isfile(idsp):
            tkMessageBox.showwarning(appName,u'Conversion of ' + replacementSound + u' to AT9 ended with an error but an AT9 was created, thus this file (which may be invalid) will be used.\nThe following command failed:\n' + cmd)
        else:
            tkMessageBox.showerror(appName,u'Conversion script to AT9 failed with ' + replacementSound + u'\nThe following command failed:\n' + cmd)
            return
...
def replace(l):
    ...
    for i in range(len(l.curselection())):
        ...
        backupSong = currentSong[:-5] + u".bak.at9"
        backupId = 0
        while os.path.isfile(backupSong):
            backupSong = currentSong[:-5] + u".bak" + six.text_type(backupId) + u".at9"
            backupId += 1
...
def revert(l):
    ...
    for i in range(len(l.curselection())):
        ...
        if not tkMessageBox.askyesno(appName,u"Are you sure you want to revert last replacement for " + os.path.basename(song) + u"?"):
            ...
            continue
        #find last backup
        backup0    = song[:-5] + u'.bak.at9'
        backupTest = backup0
        backupL    = ''
        backupId   = 0
        while os.path.isfile(backupTest):
            backupL    = backupTest
            backupTest = song[:-5] + u'.bak' + six.text_type(backupId) + u'.at9'
            backupId   += 1
        if backupL == u'':
            revertedNotN    += 1
            revertedNotName += u'\n' + os.path.basename(song)
...
def saveNus(newfile):
    ...
    if newfile:
        ...
        if os.path.realpath(nusPath) != os.path.realpath(newNusPath):
            ...
            nusPath=newNusPath
    for song in songs:
        if not os.path.isfile(song[:-5] + u'.bak.at9'):# check if song has been replaced in the current session
            continue
...
def exportidsp(l):
    ...
    for i in l.curselection():
        ...
        if os.path.isfile(currentSong):
            output = xis.decode(asksaveasfilename(defaultextension=u'.wav',initialfile=os.path.basename(currentSong)[:-5],filetypes=[(u'Wave Audio File', u'.wav'),(u'IDSP Track', u'.at9')]))
            if output ==u"":
...
def about():
    aboutText =    u'A GUI that injects AT9 sounds into nus3bank files'
    aboutText += u'\n'
    aboutText += u'\nStarted with love by jam1garner'
    aboutText += u'\nContinued with passion by BlueDan'
    aboutText += u'\nAT9 (for Taiko Vita) fork by DespairOfHarmony'
    aboutText += u'\n'
    aboutText += u'\nCredits go to :'
